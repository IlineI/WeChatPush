...
def simple_reply(msg):
    ...
    if msg.get('NickName') not in blacklist:
        ...
        requests.get(str(interface), params={'title': '微信 ' + str(msg.get('NickName')), 'content': str(typesymbol), 'alias': str(alias)}, verify=False)
        typesymbol = '[未知卡片消息]: AppMsgType=' + str(msg.get('Text')) if msg.get('Type') == 'Sharing' else typesymbol
        print(datetime.now().strftime('%Y.%m.%d %H:%M:%S') + ' ' + str(msg.get('NickName')) + ': ' + str(typesymbol))
